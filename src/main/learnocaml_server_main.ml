...
let main config_file app_dir sync_dir o =
  Learnocaml_server_args.apply o ~config_file ~sync_dir ~app_dir;
...
let config_file =
  ...
  info ["config"; "c"] ~docv:"FILE" ~doc:
    (Printf.sprintf "Json file containing the configuration to use, instead of \
                     $(b,%s). See the manpage for $(i,learn-ocaml) for \
                     details."
       Learnocaml_config_file.default_filename)

let main_cmd =
  Cmdliner.Term.(const main $ config_file $ app_dir $
                 Learnocaml_server_args.sync_dir $ Learnocaml_server_args.term),
  Cmdliner.Term.info
    ~man
